...
# Register 'mse' as a custom metric using keras.saving.register_keras_serializable
@tf.keras.saving.register_keras_serializable()
def mse(y_true, y_pred):
    return tf.reduce_mean(tf.square(y_true - y_pred))

# Load the model with 'mse' registered as a custom object
model = tf.keras.models.load_model('trained_model.h5', custom_objects={'mse': mse}, compile=False)

# Recompile the model with the correct optimizer and loss function
...
try:
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            # Run upper body estimation on the frame
            ...
            # Display the frame with upper body estimation
